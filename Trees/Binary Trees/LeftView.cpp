...
public:
    ...
    vector<int> leftView(Node* root) {     // TC: O(N)  SC: O(H)
        vector<int> ans;
        leftViewHelper(root, 0, ans);
        return ans;
    }

    void leftViewHelper(Node* node, int level, vector<int> &ans) {
        if (node == NULL) {
            return;
        }
        if (level == ans.size()) {      
        ...
        leftViewHelper(node->left, level+1, ans);
        leftViewHelper(node->right, level+1, ans);
    }
};
...
int main() {
    ...
    vector<int> ans = tree.leftView(tree.root);
    for (int i: ans) {
        cout << i << " ";
    }
    cout << endl;
